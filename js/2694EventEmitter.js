...
// when the event is emitted. An event should be able to have multiple listeners
// for the same event. When emitting an event with multiple callbacks, each should
// be called in the order in which they were subscribed. An array of results
// should be returned. You can assume no callbacks passed to subscribe are
// referentially identical. The subscribe method should also return an object
// with an unsubscribe method that enables the user to unsubscribe. When it is
// called, the callback should be removed from the list of subscriptions and
...
emitter = new EventEmitter();
sub1 = emitter.subscribe("firstEvent", (...args) => args.join(","));
console.log(emitter.emit("firstEvent", [1, 2, 3])); // ["1,2,3"]
sub1.unsubscribe(); // undefined
...
emitter = new EventEmitter();
sub1 = emitter.subscribe("firstEvent", (x) => x + 1);
sub2 = emitter.subscribe("firstEvent", (x) => x + 2);
sub1.unsubscribe(); // undefined
...
sub1 = emitter.subscribe("firstEvent", (x) => x + 1);
sub2 = emitter.subscribe("firstEvent", (x) => x + 2);
sub3 = emitter.subscribe("firstEvent", (x) => x + 3);
sub2.unsubscribe();
sub3.unsubscribe();
